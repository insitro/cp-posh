...
import cp_posh.ssl.data as datasets
...
def get_train_loader(cfg: DictConfig):
    ...
    train_data = getattr(datasets, train_data_cfg.name)(
        transform_cfg=cfg.transformations, **train_data_cfg.args
    )
    ...
    with open_dict(cfg):
        ...
        # the learning rate)
        cfg.data.loader.num_batches = (
            len(train_loader) // (cfg.trainer.num_nodes * cfg.trainer.devices) + 1
        )

...
from cp_posh.ssl import utils
from cp_posh.ssl.backbone import vision_transformer
...
class MultiCropWrapper(nn.Module):
    ...
    def __init__(self, backbone, head, channel_drop: float = 0.0):
        ...
        if channel_drop > 0:
            ...
            self.channel_drop = nn.Identity()  # type: ignore
...
class DINO(pl.LightningModule):
    def __init__(self, main_dino_cfg: DictConfig) -> None:
        ...
        student = getattr(vision_transformer, self.cfg.backbone.name)(
            **self.cfg.backbone.args
        )
        # we will set drop path rate to 0 for the teacher network
        teacher = getattr(vision_transformer, self.cfg.backbone.name)(
            drop_path_rate=0.0,
            **{
                k: v
    ...
    def set_cfg(self, main_dino_cfg: DictConfig) -> DictConfig:
        ...
        with open_dict(cfg):
            cfg.total_batch_size = (
                main_dino_cfg.trainer.num_nodes
                * main_dino_cfg.trainer.devices
                * main_dino_cfg.data.loader.batch_size
            )
